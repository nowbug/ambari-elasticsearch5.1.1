...
class Master(Script):
    ...
    def configure(self, env):
        ...
        configuration1 = config['configurations']['elastic-config']

        list1 = ['path_logs', 'path_data', 'gateway_recover_after_nodes', 'discovery_zen_minimum_master_nodes',
                 'discovery_zen_ping_unicast_hosts', 'action_destructive_requires_name', 'node_attr_rack', 'http_port',
                 'bootstrap_memory_lock', 'cluster_name', 'network_host']
        dict1 = {}
        for k, v in configuration1.items():
            if k not in list1:
                dict1[k] = v
        ...
        File(params.elastic_sysconfig_file, content=sysconfig_content, owner=params.elastic_user,
             group=params.elastic_group)

        # write /etc/elasticsearch/jvm.options
        jvm_content = InlineTemplate(params.elastic_jvm_content)
        File(format("{elastic_conf_dir}/jvm.options"), content=jvm_content, owner=params.elastic_user,
             group=params.elastic_group)

        # write /etc/elasticsearch/log4j2.properties
        log4j_content = InlineTemplate(params.elastic_log4j_content)
        File(format("{elastic_conf_dir}/log4j2.properties"), content=log4j_content, owner=params.elastic_user,
             group=params.elastic_group)
